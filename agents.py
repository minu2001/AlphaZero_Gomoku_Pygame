# agents.py (get_q_value 함수 추가 및 버그 수정 완료 버전)
...
class ZeroAgent(Agent):
    ...
    def get_pi(self, root_id, tau):
        ...
        for action_index in self.tree[self.root_id]['child']:
            ...
            if child_id in self.tree:  # 키가 존재하는지 확인
    ...
    def _selection(self, root_id):
        ...
        while self.tree.get(node_id) and self.tree[node_id].get('child'):
            ...
            children = self.tree[node_id]['child']
            if not children:
                break

            for action_idx in children:
                child_id = node_id + (action_idx,)
                if child_id in self.tree:
                    total_n += self.tree[child_id]['n']

            if total_n == 0:  # 모든 자식의 방문 횟수가 0이면
                # 정책 확률(p)에 따라 선택
                probs = [self.tree[node_id + (idx,)]['p'] for idx in children]
                action_index = children[np.argmax(probs)]
                node_id = node_id + (action_index,)
                continue

            for action_index in children:
            ...
            if not qu: break  # 계산된 QU가 없으면 중단
    ...
    def _expansion_evaluation(self, leaf_id, win_index):
        ...
        if win_index == 0:
            ...
            else:  # 모든 합법적인 수의 정책 확률이 0이면 균등 분배
                for action_index in actions:
                    prior_prob[action_index] = 1 / len(actions)

            if self.noise and leaf_id == self.root_id:
                noise_probs = np.random.dirichlet(
            ...
            for i, action_index in enumerate(actions):
                ...
                self.tree[child_id] = {'child': [], 'n': 0., 'w': 0., 'q': 0., 'p': prior_p}
    ...
    def _backup(self, leaf_id, value, reward):
        ...
        while True:
            ...
            parent_id = node_id[:-1]
            if not parent_id:  # 부모가 없으면(최상위 노드) 멈춤
                break
            node_id = parent_id
    ...
    def get_pv(self, root_id):
        ...
        self.model.eval()
        with torch.no_grad():
            state_input = torch.tensor(np.array([state])).to(device).float()
            policy, value = self.model(state_input)
            p = policy.data.cpu().numpy()[0]
            v = value.data.cpu().numpy()[0]
        return p, v

    # NEW: MCTS 계산 후의 Q value(승률)를 반환하는 함수
    def get_q_value(self, root_id):
        if root_id in self.tree:
            return self.tree[root_id]['q']
        return 0


# ... (이하 PUCTAgent, UCTAgent 등 나머지 코드는 기존과 동일) ...
# ...
# ...
